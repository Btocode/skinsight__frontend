...
jest.mock("@/components/common/Button", () => ({
  ...
  default: ({
    ...
    <button data-testid="scroll-button" className={className} onClick={onClick}>
      {children}
    </button>
  ),
}));
...
jest.mock("next/image", () => ({
  ...
  default: ({
    ...
    <img
      src={src}
      alt={alt}
      data-testid={`gallery-image-${alt
        .toLowerCase()
        .replace(/\s+/g, "-")}-${src.split("/").pop()?.replace(/\./g, "-")}`}
      className={className}
...
describe("ProductGallery", () => {
  ...
  it("renders the gallery with correct heading", () => {
    ...
    expect(screen.getByText("See it in action")).toHaveClass(
      "text-base leading-[24px] tracking-[-0.02em] lg:text-2xl font-semibold mb-4 lg:mb-6 text-accent"
    );
  });
  ...
  it("renders gallery images with correct attributes", () => {
    ...
    // Check another image
    const howToUseImage = screen.getByTestId(
      "gallery-image-how-to-use-img2-png"
    );
  ...
  it("opens the gallery modal when an item is clicked", () => {
    ...
    const galleryItemContainer = screen
      .getByTestId("gallery-image-review-img1-png")
  ...
  it("renders the scroll container with correct classes", () => {
    ...
    const scrollContainer =
      screen.getByTestId("scroll-button").parentElement?.firstChild;
    expect(scrollContainer).toHaveClass(
      "flex gap-2 lg:gap-4 overflow-x-auto scroll-smooth hide-scrollbar"
    );
  });
  ...
  it("renders each gallery item with correct structure", () => {
    ...
    expect(outerCircle).toHaveClass(
      "w-[50px] h-[50px] lg:w-[60px] lg:h-[60px] rounded-full border-2 border-rose-300 flex items-center justify-center overflow-hidden mb-2"
    );

    // Check the inner circle
    const innerCircle = outerCircle?.firstChild;
    expect(innerCircle).toHaveClass(
      "w-[45px] lg:w-[55px] h-[45px] lg:h-[55px] rounded-full border-2 border-gray-50 overflow-hidden relative"
    );
